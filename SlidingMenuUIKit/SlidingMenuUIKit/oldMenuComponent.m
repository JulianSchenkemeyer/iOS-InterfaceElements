...
@property (nonatomic, strong) UIDynamicAnimatior *animator;
...
- (id)initMenuWithFrame:(id)frame targetView:(id)targetView direction:(MenuDirectionOptions)direction options:(NSArray *)options optionImages:(NSArray *)optionImages
{
    if (self = [super init]) {
...
// Setting up the menu out of the screens visible area, so it can swoop in from the side
- (void)setupMenuView
{
...
// Setting up the Background, which prohibites touches on anything other than the swooped in menu
- (void)setupBackgroundView
{
...
- (void)setupOptionsTableView {
    self.optionsTableView = [[UITableView alloc] initWithFrame:CGRectMake(0.0, 0.0,
                                                                          self.menuFrame.size.width,
                                                                          self.menuFrame.size.height)
